    ...
    ELEMENTS.GO_BTN.addEventListener('click', () => {
      ...
      if (
        ...
        const _controlTheGame = (function (info, players, boardArr) {
          ...
          function _showWinner(type) {
            ...
            if (type?.crossReverse || type?.crossReverse) {
              ...
              boardArr.forEach((box, i) => {
                document
                  .querySelector(
                    `[data-row="${i}"][data-col="${reversedIndex}"]`
                  )
                  .classList.add('box-shown-winner')
                reversedIndex--
              })
          ...
          // Resetting the round ...
          function _resetRound(arr) {
            console.log(arr.map((row) => row.map((col) => '')))
            document.querySelectorAll('.box').forEach((box) => {
              box.classList.remove('box-shown')
              box.classList.remove('box-shown-winner')
              box.textContent = ''
            })
          }
